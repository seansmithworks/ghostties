...
Beneath the field sits the RESOLUTION LINE (`resolutionLine`, `SessionComposerPalette.swift`) — a single row reading `→ project · branch · template`, showing what the CURRENT field text resolves to. It answers "where will this go?" even with an empty field (Sean: "when hitting Cmd+T I have an intent") — `selectedOption` is seeded to the project's default template the instant the composer opens, so the template segment is never blank. It is the ONLY mouse entry point left into the project/branch pickers (browse-don't-recall) — clicking the project or branch segment toggles that segment's inline picker exactly as the deleted chips did. The template segment is plain text, not clickable — template choice stays in the results list below, not a third picker.

Kept deliberately quiet: plain `subtitleFontSize` text at `.secondary`, no pill background, no border, no width cap — a status line, not a control strip (this is the resting, always-visible form of Sean's "maybe on hover the chips are shown" ask — model B, an inline-styled live-text field, is the later finish that isn't built yet). The leading `→` and the `·` separators between segments are `.tertiary`, `.accessibilityHidden`. The project segment has two states, keyed off `isProjectLocked`, mirroring the deleted chip's own split: locked renders plain, non-clickable text (`.locked` project — a locked composer must never expose a live picker affordance), falling back to "Project unavailable" if the locked project can't resolve; unlocked is clickable, falling back to "Select project" — the only project affordance in the composer, reachable with zero projects and after the resolved project changes. The branch segment renders only when the current project is a git repo at all (`SessionComposerStore.isGitRepo`) — a non-git project shows no branch segment, not a disabled one. Three states beyond the ordinary resolved-branch name: **"Default"** (no override picked), **"Creating…"** (a `git worktree add` is in flight), and — new in this rebuild — a legible `branch "<token>" not found` message in `.systemRed` when a typed branch is unresolved, so that failure is visible on the resolution line itself rather than only after a failed Return.
