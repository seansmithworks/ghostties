...
The composer's project control is a breadcrumb CHIP (`projectChip`, composer breadcrumb spec Slice A) at the HEAD of `queryRow`, not a trailing control — this replaced the old trailing `projectControl` (hairline divider + label + chevron) so the project no longer competes with the search field for width as a second control; the chip renders inline instead, followed by a `›` separator and the field. The `queryRow` HStack spacing is 8pt (up from 0 — the old trailing control's `HStack` had no spacing at all) to give the chip/`›`/field trio enough room to read as separate elements at the new leading position. It has two visual states, keyed off `isProjectLocked`. Locked renders plain secondary-styled text with `.regular` weight and no pill background — a pill with nothing behind it reads as a control that isn't one, so it's omitted deliberately, same reasoning this section recorded for the old divider-plus-chevron control. Locked is NOT always present: if the locked project can't resolve (e.g. deleted mid-composer), the chip renders the same visible "Project unavailable" text VoiceOver already announced, rather than an empty label with a dangling `›` — a locked composer must never expose a live picker affordance (round-2 review, F5), but it must also never render as a bare gap that reads as a rendering bug. Unlocked renders a pill at `.medium` weight (the locked/unlocked split is a deliberate weight step, not an oversight): `WorkspaceLayout.composerChipBackground`/`composerChipBackgroundActive` (resting/open), `cornerRadius: 5` via `.clipShape(..., style: .continuous)` (matches `ComposerRow`'s existing row-highlight radius), horizontal padding 8pt / vertical 4pt. The unlocked state is always present, even with no project selected — an unresolved chip shows a "Select project" placeholder rather than disappearing, since it is the ONLY project affordance in the composer once the trailing control was removed (it must stay reachable with zero projects and after backspacing a chip back to text). Both states truncate the label (`.lineLimit(1)`, `.truncationMode(.tail)`) but carry NO width cap — three prior rounds shipped `.frame(maxWidth: 96)` on the chip's `Text` and each time it was wrong: a flexible `.frame(maxWidth:)` is greedy, not a ceiling, so it clamped the *proposed* width, not the content width, and rendered every project name at the identical fixed slab regardless of length. With no frame and no `.layoutPriority` set on either `HStack` child, the `Text` reports its own ideal width and the query field — which has no maximum — absorbs the rest, so a short name hugs its pill and a long name only truncates when the row genuinely runs out of room. Project names come from `url.lastPathComponent` of a user-picked folder, so long names are the normal case this has to handle gracefully. Type size is `fieldFontSize` (matches the query field, both presentations) so the breadcrumb reads as one line rather than two competing scales. The inline picker's keyboard-highlighted row reuses `WorkspaceLayout.composerChipBackground` for its highlight background (never a re-inlined `Color.secondary.opacity(0.15)`) and carries `.accessibilityAddTraits(.isSelected)`, so VoiceOver gets an equivalent of the visible keyboard highlight.

The BRANCH chip (`branchChip()`, composer breadcrumb spec Slice B) sits immediately after the project chip's own `›` separator, followed by its own `›` before the search field — same pill, same `.medium` weight, same `WorkspaceLayout.composerChipBackground`/`composerChipBackgroundActive` resting/open pair, same 5pt `.continuous` radius, same 8pt/4pt padding, same no-width-cap rule as the project chip (documented above — `.frame(maxWidth:)` is greedy, never a ceiling). It is NOT always present: it renders only when the current project is a git repo at all (`SessionComposerStore.isGitRepo`, derived from the UNFILTERED `git worktree list --porcelain` result, not from whether anything was left to OFFER after filtering) — a non-git project shows no chip, not a disabled one, matching the project chip's own "dead control is worse than no control" reasoning. Two states beyond the ordinary resolved-branch label: **"Default"** (no override picked — the picker's own "Default (`<branch>`)" row uses the same word) and **"Creating…"** (a `git worktree add` is in flight; the chip is still clickable while this shows, by design — see the branch picker's own doc comment for why a pick made mid-creation must survive that creation's later failure/timeout). No `.locked` state exists for the branch chip — the composer never locks WHICH branch launches, only whether the chip appears at all.
