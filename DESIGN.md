...
Board row metrics (6pt vertical / 10pt horizontal padding, 6pt radius, `.centered` only, Composer UI 11 Step 2) remain in tension with this section's 4pt spacing scale and one-radius rule — an open call for Sean, not resolved here; `.anchored` keeps its unstyled legacy values.
