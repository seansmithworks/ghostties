...
**Trailing picker controls** (`projectControl`/`branchControl`, plain SwiftUI Buttons trailing the field, no in-field hit testing). This is now the ONLY mouse entry point into the project/branch pickers. `projectControl` — a `chevron.down` glyph, `.tertiary`, subtitle scale, 16pt hit target — toggles the inline project picker exactly as the deleted segment did. **Hidden, not disabled, when the project is locked** — the locked rule survives verbatim: a locked composer must never expose a live picker affordance. `branchControl` — an `arrow.triangle.branch` glyph — renders only when the project is a git repo at all (`SessionComposerStore.isGitRepo`); a non-git project shows no branch control, not a disabled one. It carries a text label beside the glyph only "when it has news": the override branch name when one is picked, or `Creating…` while a `git worktree add` is in flight. No label means the default branch — the word "Default" is never restated outside the rest-state ghost placeholder. The label truncates (`.lineLimit(1)`/`.truncationMode(.tail)`) rather than being width-capped with `.frame(maxWidth:)`, which is greedy — a max-only frame takes whatever width is offered up to the max rather than hugging its content, which stretched a short branch name into dead space in an earlier line of this composer's build-out.
