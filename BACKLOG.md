...
- [ ] All above owe a local test run + red proofs (Dev was open; `build-for-testing` only). Includes the two new overlay-y-offset tests, the round-trip test, the beat-speed frame-index test, and (session-7 round 2) the `floatOffsetX` tests, the new witness-float-horizontal fallback/round-trip tests, and the updated 20-control single-line count.
