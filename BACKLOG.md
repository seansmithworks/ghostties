...
- [x] Sean's DialKit Copy-output defaults applied to code (ghost size 24→30, ghost gap 5→6, float amplitude 0→2) — style, zero-chrome defaults, and the stale `.long` shadow label left untouched per brief.
- [x] New Witness dial: float horizontal (`ghostties.composerWitnessFloatHorizontal`, model field `witnessFloatHorizontalAmplitude`, 0...4pt default 2) — sideways sway at half the vertical float's frequency (reuses the Float period dial), wired end-to-end through model/read/write/reset/panel/pill. Dial order now Ghost size → Ghost gap → Float vertical → Float horizontal → Float period → Ghost opacity → Beat speed.
- [x] Gray Witness mesh lines at size 30: `WitnessSprite`'s per-cell fill used `.rounded(.up)` widths, so 2.5pt cells drawn at 3pt overlapped neighbours by 0.5pt and double-painted the semi-transparent placeholder colour at every seam. Fixed with `WitnessSpriteGeometry.cellRect`, which snaps cell edges to the display pixel grid so adjacent cells share an exact edge, plus grouping cells by colour into one `Path` fill per colour (body cells, then eyes/lit) so each colour's alpha applies exactly once.
- [ ] All above owe a local test run + red proofs (Dev was open; `build-for-testing` only). Includes the two new overlay-y-offset tests, the round-trip test, the beat-speed frame-index test, and (session-7 round 2) the `floatOffsetX` tests, the new witness-float-horizontal fallback/round-trip tests, the updated 20-control single-line count, (session-7 round 3) the cursor-accessory-placement override test, (session-7 round 4) the `WitnessSpriteGeometry` cell-snapping tests, and (session-7 round 5, 2026-09-13 later) the single-line-default fallback/anchored-gate tests in `ComposerZeroChromeStyleTests.swift` and the rewritten `tabPlusIdentityChange…`/`unknownBranchPlusIdentityChange…` + new Reduce Motion combined-transition test in `ComposerWitnessTests.swift`, and (session-7 round 6, review closure) the extracted `SessionComposerPalette.effectiveStyle` tests plus the added `resolveFromColors` assertion on `unknownBranchPlusIdentityChange…` — `build-for-testing` only, Dev running.

- [x] DialKit single-line dial set rebuilt in `57b0fcd42` (independent review: pass). Order: Style, Treatment, Glass tint (new, default None), Width, Corner radius (new, default 10), Field size, Row size, Shadow ×4, Witness, Ghost gap (new, default 4), Reset single-line (new). Zero-chrome dials show only under Zero chrome. (Classic, and its "Style only" list, removed session-7, 2026-09-13.) Tests compiled, NOT run. The visible-controls test asserts counts only, because DialKit's control labels are `package`-scoped.
...
- [x] (decided 2026-09-13 later) Sean decided single-line is the default composer style for everyone — done, `ComposerStyle.current(defaults:)` falls back to `.singleLine` (`ComposerZeroChromeStyle.swift` ~26-33), plus matching `@AppStorage` defaults. **Classic removal: done this session (2026-09-13, session-7).** `.classic` deleted from `ComposerStyle`; a stale stored `"classic"` falls back to `.singleLine` through the existing unknown-raw-value path. The sidebar popover keeps its dedicated card (`SessionComposerPalette.popoverComposerCard`, formerly `classicComposerCard`), now selected by presentation via `SessionComposerPalette.cardKind(style:presentation:)`, not by `ComposerStyle`. Zero-chrome is untouched. `build-for-testing` + Release build both green; no `xcodebuild test` run (Dev running) — see the post-merge red-proof list above.
...
- [x] **Decided (Sean, 2026-09-13): 7 real public repos, cloned** — ghostties, impeccable-swift, agent-skills, riff, vista-sheet, surface-fx, colophon; no fakes; replaces the 10-fixture set (annotie, brukas, switchboard are PRIVATE and were leaking into captures). Branch `feat/demo-open-source-fixtures`.
  - [x] **F1 build:** (2026-09-13, commit `b5d522b97`, pushed to `origin/feat/demo-open-source-fixtures`) seed clones the 7 at pinned SHAs, 4 staged sessions remapped (ghostties/riff/surface-fx/colophon), extra branches on impeccable-swift/agent-skills/vista-sheet, `--check` 7/7 + 4/4, isolation proof re-keyed to the "Demo Agent — " session prefix, old 10-fixture dirs removed. **Found and flagged, not fixed:** all 10 old fixture dirs had `.ghostties/tasks/*.md` overlays feeding `choreograph.sh`/`terminal-flow.sh` and a task-first zone capture — no replacement content authored for the 7 new repos (per brief); those two scripts are now orphaned pending a decision (see open decision item below).
  - [x] **F1a re-pin:** (2026-09-13, follow-up commit — see below) staged agents are asked to "explain the most recent git commit," so the 4 staged repos' original tip pins were mostly poor marketing content (ghostties = a backlog-doc merge, surface-fx = a license-add, colophon = a backlog note; only riff's tip was already good). Re-pinned to each repo's most recent substantive feat/fix touching source: ghostties `05003c0` (feat(dev-badge)), riff `e39b54f` (unchanged — Fix focus/click cancel-out), surface-fx `9bcf3df` (feat(tooling): CLI/MCP/playground), colophon `66b55fe` (its root commit — the two more recent substantive candidates both had `sean@seansmithdesign.com` in a package.json diff-context line, so this is the newest one that's actually PII-clean). `--check` still 7/7 + 4/4 after re-pin; cache handled all 4 as hits (already within the depth-50 window).
  - [x] **F2 review:** separate reviewer against the full diff. (2026-09-13, reviewer PASS-WITH-NOTES on origin/main..7480d493b; C6 note — surface-fx pin 9bcf3df mentions seansmithdesign.com, public domain, its agent gets the structure prompt)
    - [x] **F2a fixes:** staged prompt bound per repo in DEMO_PROJECT_SPECS; examples README path corrected (this commit)
  - [ ] **F3 capture:** main thread only, Sean hands-off; inspect light + dark (no private names/paths, agent output from a real commit). (carried 2×)
    - capture run 1 (2026-09-13): 2/2 pass, NOT postable — surface-fx pane stuck on Claude Code's project-MCP approval dialog (fixed: disabledMcpjsonServers seeded, this commit); in-frame pane is whichever staged row is relaunched last (sidebar order), not a chosen one — pending Sean's pick
    - 2026-09-13 wrap: the MCP fix `b0f4b76e2` has NOT been re-captured. Run 1's content wait was satisfied by the login banner (42 chars), not agent output. Sean parked the demo until beta.25 ships. Revised test plan (adversarial critique, verdict rethink, rebuilt): `~/.claude/plans-html/ghostties-demo-test-plan.html`, critique files in `~/.claude/plans-html/ghostties-demo-test-plan-gate/`. It lists rig fixes R1–R7 to build on a branch cut from post-merge main before the next capture.
    - **Blocked by #175 once it merges:** "Relaunch" becomes "Resume"/"Start Fresh", and sessions restored from disk land in a collapsed Archive (`AgentSession.swift:308`, `ProjectDisclosureRow.swift:61` on `feat/sidebar-section-vocabulary`). `DemoWorkspaceCaptureUITests` clicks "Relaunch" and will find no staged rows.
  - [ ] **F4 PR:** before/after PNGs; merge on Sean's call. (carried 2×)
  - [ ] **Decide (Sean): four test-plan decisions**, strawman on each, in the plan above:
    - (1) does #175 ship in beta.25 — strawman: settle in the merge thread; R1 and glyph checks only apply if yes;
    - (2) which agent is in frame — strawman: ghostties (README summary); colophon's `git show` prints the author name + noreply address, surface-fx's README carries the old org name and npm scope;
    - (3) #175 BACKLOG item F shows "?" on a finished agent — strawman: decide item F before any marketing capture;
    - (4) Claude Code chrome in frame (plan tier, model, statusline, auto mode) — strawman: keep.
  - [ ] `MarketingCaptureUITests.swift:113` still clicks "Relaunch" — breaks once #175 merges; no gate runs it.
  - [ ] Sean: delete the capture result bundle (screen recordings), one remaining — `rm -rf "$TMPDIR"/ghostties-demo-capture-result.*` (Claude's `rm` on `$TMPDIR` is denied).
  - [ ] Commit the test plan into `docs/plans/` once Sean has reviewed it (it lives outside the repo today).
