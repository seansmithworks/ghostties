...
- [x] R13c: DialKit preset dials fixed — root cause proven with a standalone `@Published` reentrancy probe (`sink received: 100, storage-at-this-moment=0` / `final storage: 1`, not `100`): `DialPanelState.values` sends to its Combine subject BEFORE writing storage, so the coordinator's old synchronous `state.values = derived` (inside its own `$values` sink) got silently clobbered when the OUTER, still-in-flight setter finished storing its own stale-dial value — invisible to a trace of `handle`'s arguments/`lastKnownModel` since `write(from:to:)`'s persisted-defaults diff was already correct, only `state.values` itself (what the sliders/tests read) was wrong. Fix: defer the corrective `state.values = derived` write via `DispatchQueue.main.async` so it lands as its own top-level assignment, past the outer setter's return. `dialKitShadowPresetSelectionWritesAllThreeDialsAndUpdatesModel` updated to `async` + two `Task.yield()`s to observe it. Red proof owed (Dev is open)
- [x] R13c: 3 suite failures at `b64621c98` classified — `singleLineRestStateHasCardChrome` and `singleLineFieldStaysAtTheOriginalFifteenPointScale` are an **intentional change**: `b64621c98` flipped `.singleLine`'s default treatment to `.glass` (`git log -S` confirms), and the glass background branch (`SessionComposerPalette.swift`, `singleLineComposerCard`) has no `.overlay(stroke(...))` at all, so both tests' `borderStrokePixelCount > 0` check no longer matches the new default look — a real coverage gap, not touched here (no test edited). `overlayResolvedStyleFollowsInjectedDefaultsWrite` — **inferred, run owed**: verified via `git log` that neither `SessionComposerOverlay.swift` nor the `.classic`/zero-chrome border code it exercises changed anywhere in `2e4b44c59..b64621c98`; nothing in this branch's diff explains a failure here, so it's most likely pre-existing (unrelated to R13/R13b) rather than a regression this round introduced. Not fixed — no proven cause to fix
